...
- **Comms Terminal** (messaging): Global "Command Channel" broadcast + direct messages between users + Group Channels (admin-created, with membership by manual user selection, clearance level, or rank). Polling-based real-time updates every 3 seconds. Sidebar badge shows unread DM count. Supports up to 2000 chars per message. Admins can create, manage (add/remove members), and delete channels. Admins can delete individual messages (trash icon on hover) or clear entire threads/channels (trash button in header). Users can delete their own messages.
- **Role Labels** (admin only): System roles (admin/manager/viewer) can be renamed to any custom display name (e.g., Commander/Officer/Recruit) via Settings → Role Labels. Custom names appear in the users table, user management forms, and permission selectors throughout the app.
- **Custom Roles** (admin only): Create fully custom roles with a name, description, color badge, and 5 granular permission flags (manage roster, org, channels, view activity, manage users). Roles are shown as colored badges. Users can be assigned a custom role via the Edit User dialog; permissions from custom roles are merged alongside per-user overrides and base role permissions.
- **ZIP Package Update** (admin only): Upload a pre-built `ares-update.zip` via Settings → Upload Update Package to apply updates without SSH. Server validates the zip (zip-slip protected, only allows `index.cjs` + `public/**`), backs up the old bundle, extracts new files, and restarts via `process.exit(0)` (Docker auto-restarts). Build script: `scripts/build-update-zip.sh`. Uses `adm-zip` (bundled into the server binary).
